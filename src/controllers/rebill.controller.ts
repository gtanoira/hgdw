...
// Models
import { RebillModel } from '../models/rebill.model';
...
class RebillController {
  ...
  public async InsertMissingRebill(req: Request, res: Response): Promise<Response> {

    // Obtengo el nombre del archivo y lo descargo al server
    const filename = await rebillController.saveUploadFile(req)
    .then( data => data )
    .catch( (err) => {
    ...
    await rebillService.startTransaction();
    ...
    try {
      ...
      const registers: RebillModel[] = XLSX.utils.sheet_to_json(worksheet);
      ...
      */
      ...
      for (let i = 0; i < registers.length; i++) {
        ...
        if ( i > 0 && i % 1000 === 0) {
          ...
          // Ejecutar el insert 
          await rebillController.sendMissingRebill(insertValues)
          .then(data => { 
            regsGrabados += data ;
          })
          .catch(err => { 
            rebillController.rtn_status = 503; // service unavailable
        ...
        if ('auto,manual'.indexOf(register.rebillType) < 0 ) {
          rebillController.rtn_status = 400;
      ...
      rebillController.rtn_status = rebillController.rtn_status === 200 ? 503 : rebillController.rtn_status;
      await rebillService.rollbackTransaction();  // Rollback toda la transaccion
      await rebillService.endTransaction(); // finalizar la transacción      
      return res.status(rebillController.rtn_status).send({message: `HTG-015(E): ${err.toString().replace(/Error: /g, '')}`});
    }

    // Mensaje de retorno
    rebillController.rtn_status = 200;  //ok
    let rtn_message = {message:`${regsGrabados} registro/s grabados`};

    // Procesar los últimos titulos
    if (insertValues === '') {
      rebillService.commitTransaction();  // Commit toda la transaccion
      ...
      .catch(err => {
        rebillService.rollbackTransaction();  // Rollback toda la transaccion
        rebillController.rtn_status = 503;  // service unavailable
    ...
    rebillService.endTransaction();

    // Guardar la operación en Error_logs
    errorLogsService.addError('missing_rebill', rtn_message.message, 'nocode', 0);
    
    // Retornar el resultado
    console.log('*** FIN:', rebillController.rtn_status, rtn_message);
    return res.status(rebillController.rtn_status).send(rtn_message);
  }

  // Envía el comando SQL a ejecutarse a la base de datos
  private async sendMissingRebill(valuesCmd: string): Promise<number> {
    if (valuesCmd !== '') {
      ...
      return await rebillService.insertMissingRebill(sqlCmd)
      .then( data => { 
      ...
      .catch( err => { 
        ...
        errorLogsService.addError('missing_rebill', err.toString().substring(0, 4000), 'nocode', 0)
        .then(() => null)
        .catch(() => null);
  ...
  public async saveUploadFile(req: Request): Promise<string | void> {
    ...
    const filename = `rebill_${moment().format('YYYY-MM-DD_HH-mm-ss')}.xlsx`;
...
export const rebillController = new RebillController();